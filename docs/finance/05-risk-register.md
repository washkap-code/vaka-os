...
| RISK-011 | Expense category account tenant validation needs hardening | Expenses | High | `/expenses` accepts `categoryAccountId` from request before posting | Validate account belongs to tenant and is active/category-appropriate. | Open |
...
| RISK-014 | Warehouse creation lacks observed audit event | Inventory | Low | `POST /warehouses` inserts without audit in inspected route | Add audit event when touching inventory governance. | Open |
...
| RISK-016 | Zero-value journal lines are allowed inside non-zero journals | Ledger quality | Low | `journal-invalid-lines.test.ts` executably proves a zero-value line is allowed inside a non-zero balanced journal. | Add per-line zero-value rule if accounting policy requires it. | Open |
