...
export function createApp() {
  ...
  app.use("/api/v1/auth/refresh", createRateLimiter({ windowMs: 5 * 60_000, max: 60, label: "session renewal attempts" }));
  app.use("/api/v1/auth/step-up", createRateLimiter({ windowMs: 5 * 60_000, max: 10, label: "reauthentication attempts" }));
