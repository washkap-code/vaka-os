...
// Renderer/branding provenance recorded with every snapshot. Bump these when
// `report-pdf.ts` layout or `getReportBranding` shape changes incompatibly, so
// stored snapshots fail closed instead of re-rendering with drifted output.
export const FINANCE_REPORT_PDF_VERSION = "report-pdf-v1" as const;
export const FINANCE_REPORT_BRANDING_VERSION = "tenant-report-branding-v1" as const;
...
function renderStoredSnapshot(row: SnapshotRow): Buffer {
  ...
  const branding = row.brandingDocument as unknown as ReportBranding;
...
export async function createFinanceReportSnapshot(input: FinanceReportSnapshotInput & {
  ...
  const [report, branding] = input.reportType === "VAT"
    ? await Promise.all([
      ...
      getReportBranding(input.tenantId),
    ])
    : await Promise.all([
  ...
  return db.transaction(async (tx) => {
    const [created] = await tx.insert(schema.financeReportSnapshots).values({
      ...
      brandingVersion: FINANCE_REPORT_BRANDING_VERSION,
      parameters: period,
